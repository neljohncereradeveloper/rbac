...
export function AssignPermissionsDialog({
  ...
  const [userRoles, setUserRoles] = useState<Array<{ role_id: number; role_name: string }>>([])
  const [rolePermissionsMap, setRolePermissionsMap] = useState<Map<number, number[]>>(new Map())
  const [fetchError, setFetchError] = useState<string | null>(null)
  const [isLoading, setIsLoading] = useState(false)
  const [activeTab, setActiveTab] = useState<"grant" | "deny" | "remove">("grant")
  ...
  const permissionGroups = useMemo<PermissionGroup[]>(() => {
    ...
    permissions.forEach((permission) => {
      ...
      // or use the resource field if available
      const resource = permission.resource || permission.name.split(":")[0] || "other"
  ...
  // Expand all resources by default on initial load only
  useEffect(() => {
    if (
    ...
    const loadData = async () => {
      try {
        ...
        if (user?.id && userRolesRes && userRolesRes.length > 0) {
          ...
          const rolesWithNames: Array<{ role_id: number; role_name: string }> = []
  ...
  // Calculate permissions user already has from roles
  ...
  // Effective permissions: from roles + granted overrides (excluding denied)
  ...
  function selectAllInTab() {
    if (activeTab === "grant") {
      // Only select permissions user doesn't already have
      ...
      // Only select permissions user already has
      const allIds = permissions
        .filter((p) => effectivePermissions.has(p.id))
        .map((p) => p.id)
  ...
  const selectedCount =
    activeTab === "grant"
      ...
      : activeTab === "deny"
        ? (denyPermissionIds?.filter((id) => effectivePermissions.has(id)).length ?? 0)
              ...
              <CollapsibleContent className="px-3 sm:px-4 py-2">
                <div className="space-y-1">
                  {groupPermissions.map((permission) => {
                    ...
                    const disabled = isDisabled ? isDisabled(permission.id) : false
                    return (
                      <label
                        key={permission.id}
                        className={cn(
                    ...
                    {isLoading ? (
                      ...
                      <div className="border-input max-h-[450px] overflow-y-auto rounded-lg border bg-muted/30 p-4">
                        {renderPermissionList(
                          ...
                          (permission) => {
                            const hasFromRoles = permissionsFromRoles.has(permission.id)
                            const hasGranted = grantedPermissionIds.has(permission.id)
                            const roleSources = permissionRoleSources.get(permission.id) || []
                            ...
                            if (hasGranted) {
                              ...
                              return (
                                <span className="text-xs text-blue-600 dark:text-blue-400 font-medium" title={`From roles: ${roleSources.join(", ")}`}>
                                  (From {roleSources.length === 1 ? `Role: ${roleSources[0]}` : `${roleSources.length} Roles`})
                                </span>
                              )
                            }
                            return null
                          ...
                          (id) => {
                            // Disable if user already has permission from roles or already granted
                            // User should use Deny tab to block permissions they have from roles
                            return effectivePermissions.has(id)
                          }
                        )}
                      </div>
                    )}
                ...
                <FieldGroup className="min-h-0 flex-1 flex flex-col">
                  <Field className="flex min-h-0 flex-1 flex-col">
                    <div className="flex shrink-0 items-center justify-between mb-3">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          Override to explicitly block permissions. Only shows permissions the user currently has access to.
                        </p>
                      </div>
                    ...
                    {isLoading ? (
                      ...
                      // Filter to only show permissions user already has
                        ...
                        return (
                          <div className="border-input max-h-[450px] overflow-y-auto rounded-lg border bg-muted/30 p-4">
                            <p className="text-muted-foreground text-sm py-4 text-center">
                              No permissions to deny. User doesn't have any permissions that can be blocked.
                            </p>
                          </div>
                        )
                      }
                      ...
                      return (
                        <div className="border-input max-h-[450px] overflow-y-auto rounded-lg border bg-muted/30 p-4">
                          {renderPermissionList(
                            ...
                            (permission) => {
                              const isDenied = deniedPermissionIds.has(permission.id)
                              const hasFromRoles = permissionsFromRoles.has(permission.id)
                              const hasGranted = grantedPermissionIds.has(permission.id)
                              const roleSources = permissionRoleSources.get(permission.id) || []
                  ...
                  <Field className="flex min-h-0 flex-1 flex-col">
                    ...
                    {isLoading ? (
                      ...
                      <div className="border-input max-h-[450px] overflow-y-auto rounded-lg border bg-muted/30 p-4">
                        <div className="space-y-2">
                          {permissionGroups.map((group) => {
                            const groupOverrides = userPermissions.filter((up) =>
                              group.permissions.some((gp) => gp.id === up.permission_id)
                            )
                              ...
                              <div key={group.resource} className="space-y-1">
                                ...
                                {groupOverrides.map((userPermission) => {
                                  const permission = permissions.find(
                                    (p) => p.id === userPermission.permission_id
                                  )
                                  const isSelected = removePermissionIds?.includes(
                                    ...
                                    >
                                      ...
                                      <div className="min-w-0 flex-1">
                                        <div className="flex items-start gap-2 flex-wrap">
                                          ...
                                          >
                                            ({userPermission.is_allowed ? "Granted" : "Denied"})
                                          </span>
                                        </div>
                                        {permission?.description && (
                                          <p className="text-muted-foreground text-xs mt-1 wrap-break-word">
